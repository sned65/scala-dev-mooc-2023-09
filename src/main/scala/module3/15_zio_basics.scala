...
object zioConstructors {
  ...
  lazy val z4: Task[Int] = ZIO.fromFuture(implicit ec => f.flatMap(i => Future(i + 1)))
  ...
  lazy val _: UIO[Nothing] = ZIO.die(new Throwable("oops"))
  lazy val _: IO[String, Nothing] = ZIO.fail[String]("Error")
...
object zioOperators {
   ...
   */
  lazy val ab1: ZIO[Any, Throwable, (Unit, String)] = a.zip(b)
   ...
   * последовательная комбинация эффектов a и b
   */
  lazy val ab3: Task[Unit] = a.zipLeft(b)
  ...
  lazy val ab5: ZIO[Any, Throwable, Int] = a.as(10) // a.map(_ => 10)
