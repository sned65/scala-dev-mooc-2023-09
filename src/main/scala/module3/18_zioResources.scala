...
object tryFinally {
  ...
  object traditional {
    ...
    val result = withResource(Source.fromFile("test.txt"))(_.close()){ s =>
      s.getLines()
    }
...
object zioZManaged{
   ...
   */
  lazy val file1: ZManaged[Any, Throwable, BufferedSource] = ZManaged.make(openFile("test1.txt"))(closeFile)
  ...
  // Комбинирование
  lazy val combined: ZManaged[Any, Throwable, (BufferedSource, BufferedSource)] =
  file1 zip file2


  // Параллельное открытие / закрытие
  lazy val combined2: ZManaged[Any, Throwable, (BufferedSource, BufferedSource)] =
    file1 zipPar file2
