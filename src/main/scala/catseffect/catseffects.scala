...
object catseffects {
  def main(args: Array[String]): Unit = {
    ...
    val sideffect = IO.delay(println("111"))
//    val mistake = IO.pure(println("error"))

//    sideffect.unsafeRunSync()
//    sideffect.unsafeRunSync()
...
object  FilesAndHttpIO extends IOApp.Simple {
  ...
  def run: IO[Unit] = for {
    ...
    path <- IO.readLine
    data <- readFile(path)
    _ <- httpPost("http://blabla.de", data)
  } yield ()
}
...
object FilesAndHttpTF extends IOApp.Simple {
  def program[F[_] : Console: Monad : FileSystem : HttpClient]: F[Unit] =
    for {
      ...
      _ <- HttpClient[F].postData("sefsefr.de", data)
    } yield ()
